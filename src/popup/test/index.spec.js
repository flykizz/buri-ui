...
test('popup lock scroll', () => {
  ...
  expect(document.body.classList.contains('buri-overflow-hidden')).toBeTruthy();
  ...
  wrapper2.vm.$destroy();
  expect(document.body.classList.contains('buri-overflow-hidden')).toBeFalsy();
});
...
test('get container with parent', () => {
  ...
  const popup = wrapper.find('.buri-popup').element;
...
test('render overlay', async () => {
  ...
  expect(div.querySelector('.buri-overlay')).toBeTruthy();
});
...
test('watch overlay prop', async () => {
  ...
  await later();
  expect(div.querySelector('.buri-overlay')).toBeFalsy();
...
test('close on click overlay', async () => {
  ...
  const modal = div.querySelector('.buri-overlay');
