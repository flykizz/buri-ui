...
`buri.config.js`中包含了`buri-cli`的打包配置和文档站点配置，请创建此文件并置于项目根目录下。下面是一份基本配置的示例：
...
module.exports = {
  site: {
    versions: [
      {
        label: '1.x',
        link: 'https://flykizz.github.io/buri/1.x/'
      }
    ]
  }
};
...
推荐使用`buri-cli`内置的 preset，配置如下：

```js
module.exports = {
  presets: ['@buri-ui/cli/preset']
};
```

`@buri-ui/cli/preset`中默认包含了以下插件：
